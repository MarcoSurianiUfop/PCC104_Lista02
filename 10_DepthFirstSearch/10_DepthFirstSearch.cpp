/******************************************************************************
                  ...
                  LISTA 2 - ATIVIDADES PRÁTICAS - EXERCÍCIO 10
                                Depth-First Search
...
void dfs(Grafo& G, int& v, vector<int>& visited_vertex, int& count) {
    /*  Implementação do DEPTH-FIRST SEARCH (visita os vértices)
    * Visita recursivamente todos vértices conectados ao vértice v e os marca
    ...
    */
    ...
    for (int j = 0; j < G.num_edges(); j++) {
        ...
        // Caso a origem da aresta atual seja o vértice v
        // (ou seja, o vértice w é adjacente vértice v)
        // e caso o vértice atual ainda tenha marcação 0 (não visitado)
        if (G.get_o(j) == v && visited_vertex[w] == 0)

            // Chame recursivamente a função dfs para o vértice w
            dfs(G, w, visited_vertex, count);
    }
}

void DFS(Grafo& G) {
    /*  Implementação do DEPTH-FIRST SEARCH (itera os vértices)
    ...
    * pela primeira vez pelo dfs
    * ENTRADAS:
    * Ponteiro para objeto do tipo Grafo G = <V, E, M>
    * SAÍDA:
    * Altera o vetor de marcações M do Grafo G = <V, E, M> seguindo dfs
    */
    ...
    for (int v = 0; v < G.get_nodes(); v++)  
        ...
        if (visited_vertex[v] == 0)
            // Visite o vértice v e seus adjacentes usando dfs
            dfs(G, v, visited_vertex, count);
...
int main() {
    ...
    // Executa o algoritmo DFS, que marca automaticamente os vértices de G
    DFS(G);
